...
public class MapGenCavesX extends MapGenBase
{
    /**
    ...
    private boolean isTopBlock(byte[] data, int index, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        //System.out.println("biomeID_B:" + (int)biome.biomeID);
        //System.out.println("ファイル名2A:" + data[index]+ "/" +index);
        //System.out.println("ファイル名2B:" + biome.topBlock);
    	//BiomeGenBase biome = new BiomeRakuen(DqmNextDim.cfgDimBiomeID[DqmNextDim.idxAkuma]);
    ...
    {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        //if(biome.biomeID == DqmNextDim.cfgDimBiomeID[DqmNextDim.idxAkuma])
        //{
        //	biome = new BiomeRakuen(DqmNextDim.cfgDimBiomeID[DqmNextDim.idxAkuma]);
        //}
        int top    = biome.topBlock;
        int filler = biome.fillerBlock;
        int block  = data[index];

        //System.out.println("biomeID_A:" + (int)biome.biomeID);
        //System.out.println("ファイル名3A:" + (int)top);
        //System.out.println("ファイル名3B:" + (int)filler);
        //System.out.println("ファイル名3C:" + (int)block);


        //if (block == BlockList.DqmDimStone.blockID || block == BlockList.DqmDimBlocks_dirt[DqmNextDim.idxAkuma].blockID || block == BlockList.DqmDimBlocks_grass[DqmNextDim.idxAkuma].blockID)
        if (block == (byte)BlockList.DqmDimStone.blockID || block == top || block == filler)
        {
            if (y < 10)
            {
            ...
            {
            	//System.out.println("ファイル名3A:" + top+ "/");
                data[index] = 0;
