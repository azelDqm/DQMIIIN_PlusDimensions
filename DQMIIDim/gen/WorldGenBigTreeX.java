...
public class WorldGenBigTreeX extends WorldGenBigTree
{
    /**
    ...
    public WorldGenBigTreeX(boolean par1)
    {
        super(par1);
    }
     ...
     */
    public void generateLeafNodeList2()
    {
        ...
        while (i1 >= 0)
        {
            ...
            else
            {
                for (double d0 = 0.5D; j1 < i; ++j1)
                {
                    ...
                    if (this.checkBlockLine2(aint1, aint2) == -1)
                    {
                        ...
                        if (this.checkBlockLine2(aint3, aint1) == -1)
                        {
                            aint[k][0] = k1;
                            aint[k][1] = j;
                            aint[k][2] = l1;
    ...
    public void genTreeLayer2(int par1, int par2, int par3, float par4, byte par5, int par6)
    {
    	int idx = 8;
    	BiomeGenBase biome = this.worldObj.getBiomeGenForCoords(par1, par3);
    	for (int c = 0; c < DqmNextDim.cfgDimBiomeID.length; c++)
    	{
    		if (DqmNextDim.cfgDimBiomeID[c] != 0 &&DqmNextDim.cfgDimBiomeID[c] == biome.biomeID)
    		{
    			idx = c;
    			break;
    		}
        ...
        for (aint1[par5] = aint[par5]; j1 <= i1; ++j1)
        {
            ...
            while (k1 <= i1)
            {
                ...
                else
                {
                    ...
                    if (l1 != 0 && l1 != BlockList.DqmBlockLeaves.blockID)
                    {
                        ++k1;
                    }
                    else
                    {
                    	//System.out.println("ファイル名par6" + par6);
                        this.setBlockAndMetadata(this.worldObj, aint1[0], aint1[1], aint1[2], par6, idx);
                        ++k1;
                    }
                }
            }
     ...
     */
    public float layerSize2(int par1)
    {
    ...
    public float leafSize2(int par1)
    {
     ...
     */
    void generateLeafNode2(int par1, int par2, int par3)
    {
        int l = par2;

        for (int i1 = par2 + this.leafDistanceLimit; l < i1; ++l)
        {
            float f = this.leafSize2(l - par2);
            this.genTreeLayer2(par1, l, par3, f, (byte)1, BlockList.DqmBlockLeaves.blockID);
        }
    }
     ...
     */
    void placeBlockLine2(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, int par3)
    {
    	int idx = 8;
    	BiomeGenBase biome = this.worldObj.getBiomeGenForCoords(0, 0);
    	for (int c = 0; c < DqmNextDim.cfgDimBiomeID.length; c++)
    	{
    		if (DqmNextDim.cfgDimBiomeID[c] != 0 &&DqmNextDim.cfgDimBiomeID[c] == biome.biomeID)
    		{
        ...
        if (aint2[b1] != 0)
        {
            ...
            for (int k = aint2[b1] + b4; j != k; j += b4)
            {
                ...
                //System.out.println("ファイル名par6" + par3);
                this.setBlockAndMetadata(this.worldObj, aint3[0], aint3[1], aint3[2], par3, idx);
            }
        }
    }
     ...
     */
    boolean leafNodeNeedsBase2(int par1)
    {
    ...
    void generateTrunk2()
    {
        ...
        this.placeBlockLine2(aint, aint1, BlockList.DqmBlockLog.blockID);
    ...
    void generateLeafNodeBases2()
    {
        ...
        for (int[] aint = new int[] {this.basePos[0], this.basePos[1], this.basePos[2]}; i < j; ++i)
        {
            ...
            if (this.leafNodeNeedsBase2(k))
            {
                this.placeBlockLine2(aint, aint2, BlockList.DqmBlockLog.blockID);
            }
        }
    }
     ...
     */
    int checkBlockLine2(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger)
    {
        int[] aint2 = new int[] {0, 0, 0};
        byte b0 = 0;
        byte b1;
        ...
        else
        {
            ...
            for (j = aint2[b1] + b4; i != j; i += b4)
            {
                ...
                if (k != 0 && k != BlockList.DqmBlockLeaves.blockID)
                {
                    break;
                }
            }
    ...
    boolean validTreeLocation2()
    {
        ...
        boolean isValidSoil = (soil != null && soil.canSustainPlant(worldObj, basePos[0], basePos[1] - 1, basePos[2], ForgeDirection.UP, (BlockSapling)BlockList.DqmBlockSapling));
        ...
        {
            int j = this.checkBlockLine2(aint, aint1);
    ...
    public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5)
    {
        ...
        if (!this.validTreeLocation2())
        {
            return false;
        }
        else
        {
            this.generateLeafNodeList2();
            this.generateLeaves2();
            this.generateTrunk2();
            this.generateLeafNodeBases2();
            return true;
        }
    }
}
