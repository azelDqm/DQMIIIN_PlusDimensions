...
public class DqmWorldGenKajinoT implements IWorldGenerator
{
	private int Hougaku = 0;
	private int yMain = 0;

    public DqmWorldGenKajinoT(boolean b) {	}
    public DqmWorldGenKajinoT() {}
    public DqmWorldGenKajinoT(int face, int par2)
    {
    	this.Hougaku = face;
    	this.yMain = par2;
    }
        ...
        if(this.Hougaku == 0 || this.Hougaku == 2)
        {
	        ...
	        for (int x = -10; x <= 10; ++x)
	        {
	            for (int z = -10; z <= 10; ++z)
	            {
	                for (int y = 0; y <= 9; ++y)
	                {
	                    par3World.setBlock(xxx + x, yyy + y, zzz + (z * byVal), DqmItemList.Kowarenai4.blockID, 0, 2);
	                }
	            }
	        }
	        ...
	        par3World.setBlock(xxx + 4, yyy + 2, zzz + (6 * byVal), DqmItemList.Hepaitosu.blockID, 0, 2);
	        {
	            ...
	            {
	                int x = 0;
	                par3World.setBlock(xxx + x, yyy + 1, zzz + (z * byVal), DqmItemList.KajinoB.blockID, 0, 2);
	                par3World.setBlock(xxx + x, yyy + 2, zzz + (z * byVal), DqmItemList.Kajino.blockID, 0, 2);
	            }
	            {
	                int x = 3;
	                par3World.setBlock(xxx + x, yyy + 1, zzz + (z * byVal), DqmItemList.KajinoB.blockID, 0, 2);
        ...
        {
        	{
    	        ...
    	        for (int x = -10; x <= 10; ++x)
    	        {
    	            for (int z = -10; z <= 10; ++z)
    	            {
    	                for (int y = 0; y <= 9; ++y)
    	                {
    	                    par3World.setBlock(xxx + (x * byVal), yyy + y, zzz + z, DqmItemList.Kowarenai4.blockID, 0, 2);
    	                }
    	            }
    	        }
    	        ...
    	        par3World.setBlock(xxx + (6 * byVal), yyy + 2, zzz + 4, DqmItemList.Hepaitosu.blockID, 0, 2);
    	        {
    	            ...
    	            {
    	                int z = 0;
    	                par3World.setBlock(xxx + (x * byVal), yyy + 1, zzz + z, DqmItemList.KajinoB.blockID, 0, 2);
    	                par3World.setBlock(xxx + (x * byVal), yyy + 2, zzz + z, DqmItemList.Kajino.blockID, 0, 2);
    	            }
    	            {
    	                int z = 3;
    	                par3World.setBlock(xxx + (x * byVal), yyy + 1, zzz + z, DqmItemList.KajinoB.blockID, 0, 2);
