...
import DQMII.DqmEventKaitori;
import DQMII.DqmItemList;
import cpw.mods.fml.common.IWorldGenerator;

public class DqmWorldGenKaitoriT implements IWorldGenerator
{
	private int Hougaku = 0;
	private int yMain = 0;
    public DqmWorldGenKaitoriT(boolean b) {	}
    public DqmWorldGenKaitoriT() {}
    public DqmWorldGenKaitoriT(int face, int par2)
    {
    	this.Hougaku = face;
    	this.yMain = par2;
    }
    ...
    public void generate(Random rand, int par1, int par2, World par3World, IChunkProvider par4IChunkProvider, IChunkProvider par5IChunkProvider)
    {
        ...
        for (int x = -3; x <= 3; ++x)
        {
            for (int z = -3; z <= 3; ++z)
            {
                for (int y = -1; y <= 2; ++y)
                {
                    par3World.setBlock(xxx + x, yyy + y, zzz + z, Block.stoneBrick.blockID, 0, 2);
                }
            }
        }
        ...
        }else
        {
	        ...
	        par3World.setBlock(xxx - (2 * byV), yyy + 1, zzz + (4 * byV), DqmItemList.Taimatu.blockID, tai2, 2);
	        par3World.setBlock(xxx + (4 * byV), yyy + 1, zzz + (3 * byV), DqmItemList.Douguya.blockID, this.Hougaku, 2);
	        par3World.setBlock(xxx + (4 * byV), yyy + 1, zzz - (3 * byV), DqmItemList.Douguya.blockID, this.Hougaku, 2);
        }

        DqmEventKaitori entitysilverfish = new DqmEventKaitori(par3World);
