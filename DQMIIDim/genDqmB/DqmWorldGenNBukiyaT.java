...
import DQMII.DqmEventBukiyaHell;
import DQMII.DqmItemList;
import cpw.mods.fml.common.IWorldGenerator;

public class DqmWorldGenNBukiyaT implements IWorldGenerator
{
	private int Hougaku = 0;
	private int yMain = 0;
    public DqmWorldGenNBukiyaT(boolean b) {	}
    public DqmWorldGenNBukiyaT() {}
    public DqmWorldGenNBukiyaT(int face, int par2)
    {
    	this.Hougaku = face;
    	this.yMain = par2;
    }
    ...
    public void generate(Random rand, int par1, int par2, World par3World, IChunkProvider par4IChunkProvider, IChunkProvider par5IChunkProvider)
    {
        ...
        for (int x = -3; x <= 3; ++x)
        {
            for (int z = -3; z <= 3; ++z)
            {
                for (int y = -1; y <= 2; ++y)
                {
                    par3World.setBlock(xxx + x, yyy + y, zzz + z, Block.netherBrick.blockID, 0, 2);
                }
            }
        }
        ...
        for (int x = xStart; x <= xEnd; ++x)
        {
        	for (int z = zStart; z <= zEnd; z++)
        	{
        		par3World.setBlock(xxx + x, yyy, zzz + z, Block.stairsNetherBrick.blockID, metaV, 2);
        	}
        }
        ...
        DqmEventBukiyaHell entitysilverfish = new DqmEventBukiyaHell(par3World);
