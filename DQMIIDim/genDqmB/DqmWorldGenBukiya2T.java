...
import DQMII.DqmEventBukiyaYoru;
import DQMII.DqmItemList;
import cpw.mods.fml.common.IWorldGenerator;

public class DqmWorldGenBukiya2T implements IWorldGenerator
{
	private int Hougaku = 0;
	private int yMain = 0;
    public DqmWorldGenBukiya2T(boolean b) {	}
    public DqmWorldGenBukiya2T() {}
    public DqmWorldGenBukiya2T(int face, int par2)
    {
    	this.Hougaku = face;
    	this.yMain = par2;
    }
    ...
    public void generate(Random rand, int par1, int par2, World par3World, IChunkProvider par4IChunkProvider, IChunkProvider par5IChunkProvider)
    {
        ...
        for (int x = -3; x <= 3; ++x)
        {
            for (int z = -3; z <= 3; ++z)
            {
                for (int y = -1; y <= 2; ++y)
                {
                    par3World.setBlock(xxx + x, yyy + y, zzz + z, Block.cobblestone.blockID, 0, 2);
                }
            }
        }
        ...
        DqmEventBukiyaYoru entitysilverfish = new DqmEventBukiyaYoru(par3World);
