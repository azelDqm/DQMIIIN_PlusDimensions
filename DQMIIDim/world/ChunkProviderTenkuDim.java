...
import DQMIIDim.biomes.BiomeTenku;
import DQMIIDim.gen.MapGenRavineX;
import DQMIIDim.gen.WorldGenLakesX;

public class ChunkProviderTenkuDim implements IChunkProvider
{
    /** RNG. */
    private Random rand;
    ...
    public ChunkProviderTenkuDim(World par1World, long par2, boolean par4)
    {
    ...
    /*
        ...
        for(int cnt = 0; cnt < this.biomesForGeneration.length; cnt++)
        {
        	...
        	biomesForGeneration[cnt] = new BiomeTenku(DqmNextDim.cfgDimBiome2ID);
        }
...
*/
            ...
            for (int var30 = 0; var30 < 4; var30++)
            {
              ...
              for (int var41 = 0; var41 < 8; var41++)
              {
                ...
                for (int var50 = 0; var50 < 8; var50++)
                {
                  int var51 = 0;
                  if (var46 > 0.0D) {
                    var51 =  BlockList.DqmDimStone.blockID;
                  }
    ...
    public void replaceBlocksForBiome(int par1, int par2, byte[] par3ArrayOfByte, BiomeGenBase[] par4ArrayOfBiomeGenBase)
    {
        ...
        for (int k = 0; k < 16; ++k)
        {
            for (int l = 0; l < 16; ++l)
            {
            	...
            	BiomeGenBase biomegenbase = new BiomeTenku(DqmNextDim.cfgDimBiome2ID);
                ...
                for (int k1 = 127; k1 >= 0; --k1)
                {
                    ...
                    {
                        par3ArrayOfByte[l1] = (byte)BlockList.DqmDimGate2F.blockID;
                    }
                    else if (k1 > 30)
                    {
        ...
        for (int k = 0; k < 16; ++k)
        {
            for (int l = 0; l < 16; ++l)
            {
                ...
                {
                    int l1 = (l * 16 + k) * 128 + k1;
                    if(par3ArrayOfByte[l1] ==(byte)BlockList.DqmDimStone.blockID)
                    {
                    	par3ArrayOfByte[l1] = (byte)BlockList.DqmDimCloud.blockID;
                    }
                }
            }
        }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        for (int k = 0; k < abyte1.length; ++k)
        {
        	...
        	abyte1[k] = (byte)DqmNextDim.cfgDimBiome2ID;
        }
    ...
    private double[] initializeNoiseField(double[] ad, int i, int j, int k, int l, int i1, int j1)
    {
      ...
      this.noise5 = this.noiseGen5.generateNoiseOctaves(this.noise5, i, k, l, j1, 1.121D, 1.121D, 0.5D);
      this.noise6 = this.noiseGen6.generateNoiseOctaves(this.noise6, i, k, l, j1, 200.0D, 200.0D, 0.5D);
      d *= 2.0D;
      ...
      for (int j2 = 0; j2 < l; j2++)
      {
        ...
        for (int l2 = 0; l2 < j1; l2++)
        {
          ...
          d4 *= d4;
          d4 = 1.0D - d4;
          double d5 = (this.noise5[l1] + 256.0D) / 512.0D;
          d5 *= d4;
          if (d5 > 1.0D) {
            d5 = 1.0D;
          }
          double d6 = this.noise6[l1] / 8000.0D;
          if (d6 < 0.0D) {
            d6 = -d6 * 0.3D;
          }
          d6 = d6 * 3.0D - 2.0D;
            ...
            for (int l2 = 0; l2 < par7; ++l2)
            {
                ...
                for (int i3 = -b0; i3 <= b0; ++i3)
                {
                    for (int j3 = -b0; j3 <= b0; ++j3)
                    {
                    	...
                    	BiomeGenBase biomegenbase1 = new BiomeTenku(DqmNextDim.cfgDimBiome2ID);//this.biomesForGeneration[k2 + i3 + 2 + (l2 + j3 + 2) * (par5 + 5)];
